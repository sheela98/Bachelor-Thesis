...
cdef extern from "cpath.h":
    ...
    GreedyResult greedy_exec[T](const Expression[T] & expression, uint64_t seed, uint64_t max_repeats, double max_time, bool progbar, Minimize minimize, bool is_outer_optimal, uint32_t threshold_optimal, int cost_function_id)

class CGreedy(oe.paths.PathOptimizer):
    def __init__(self, seed = 0, minimize='size', max_repeats = 16, max_time = 0.0, progbar = False, is_outer_optimal = False, uint32_t threshold_optimal = 12, int cost_function_id = -1):
        """
    ...
    def __call__(self, inputs, output, sizes, memory_limit=None):
        ...
        tic = time.time()
        result = greedy_exec[uint32_t](expression, self.seed, self.max_repeats, self.max_time, self.progbar, minimize, self.is_outer_optimal, self.threshold_optimal, self.cost_function_id )
        toc = time.time()
